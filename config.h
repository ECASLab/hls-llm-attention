...
using StreamSingleT = hls::stream<RawSingleDataT>;
  ...
  static const DataT NEG_INF = -std::numeric_limits<float>::infinity();
#elif defined(USE_FLOAT16) || defined(USE_FLOAT8) || defined(USE_FLOAT4)
  static const DataT NEG_INF = DataT(-std::numeric_limits<float>::infinity());
#else
  static const DataT NEG_INF = DataT(-((DataT)1 << (kFxPDataInt - 1)));
#endif
